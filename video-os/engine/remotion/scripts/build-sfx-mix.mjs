// build-sfx-mix.mjs — bake each NIF003 beat's SFX cues into ONE mixed mp3.
//
// Reads NIF003_SFX from src/nif003/v4/audio.tsx (the single source of truth),
// and for each beat writes public/audio/nif003/sfx-mix/B##.mp3 — every one-shot
...
//
//   node scripts/build-sfx-mix.mjs           # all 33
//   node scripts/build-sfx-mix.mjs B01,B29   # just these
...
const FPS = 30;
const SFX_DIR = "public/audio/nif003/sfx";
const OUT_DIR = "public/audio/nif003/sfx-mix";
mkdirSync(OUT_DIR, { recursive: true });

// pull NIF003_SFX + BEAT_STARTS out of audio.tsx without importing TS
const src = readFileSync("src/nif003/v4/audio.tsx", "utf8");
const grab = (name) => {
...
const SFX = grab("NIF003_SFX");
const STARTS = grab("BEAT_STARTS");

// beat durations from reconcile.json (for padding the mix to full length)
const recon = JSON.parse(readFileSync("../../episodes/NIF003/03_transcript/reconcile.json", "utf8"));
const DUR = Object.fromEntries(Object.values(recon.beats).map((b) => [b.id, b.durationInFrames]));
...
for (const beat of beats) {
  ...
  // pad to just past the last cue's tail, but never past the beat's own end
